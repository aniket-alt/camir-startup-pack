...
| **Cost / time** | ~120 GPU-hours ≈ **$300** `(assumption: derived from [S26][S27] rates)` · 2 weeks · 2026-09-14 → 2026-09-28 |
| **Result** | — `planned` |
| **Learning** | — |
...
| **Design** | Run E1 twice on identical generations. **Naive arm**: fixed 512-token generation budget, strict single-format parse, single judge at default temperature. **Controlled arm**: generation budget raised until the truncation rate falls below 1% with truncation logged per prompt; multi-format tolerant parser with parse-failure counts reported; two judges at temperature 0 with position fixed and length normalised, with inter-judge agreement published alongside every curve [S33][S34]. The delta between the arms *is* the result. |
...
| **Design** | Take the E1 prompt set with its natural repeat structure. Apply an embedding-similarity semantic cache [S37] at a threshold tuned to a **30% hit rate**, the midpoint of the 20–45% production range [S36]. Recompute the oracle ceiling and the cascade frontier on the surviving cache-miss traffic only. Compare achievable saving per remaining request against the uncached run. |
...
| **Threshold** | **PASS**: zero reverts, and any quality question answered from traces within **1 hour**. **FAIL**: any revert — the P0 set is insufficient and the structural asymmetry [../strategy/personas.md](../strategy/personas.md) identifies (saving accrues to the budget owner, risk accrues to a different reporting line) is not solvable in product alone. |
