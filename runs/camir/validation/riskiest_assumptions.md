...
The rows that decide whether CAMIR exists are **A2** (is there anything to route toward), **A3** (is there anyone to sell to) and **A4** (will they pay for it). **Row 1b is the one that decides whether CAMIR is different from every other router**: the pack's re-scoped technical claim. It is co-ranked with A2 rather than numbered separately because it is decided by the same generations, in the same two weeks, at no extra cost — by this board's own rule it cannot rank lower. An earlier version folded it into A13 at rank 14, which tested the team's background rather than the claim.
...
| **1b** | **The harness thesis — a meaningful part of the apparent oracle ceiling on a self-hosted pool is evaluation artifact, not capability** ([../HANDOFF.md](../HANDOFF.md) §2.2) | **reshapes fatally — it is the wedge** | For: [S5] across 206,000 query-model pairs finds truncation in 65% of MMLU / 57% of MedQA cases and 5–12% MMLU parse failures. Against: none on a self-hosted pool, and a well-instrumented harness should show a smaller share — the effect is expected to shrink as harness quality rises | **E2** — the same generations as E1, run naive and artifact-controlled | inside E1 | **2 weeks, same window as rank 1** | `untested` |
...
<!-- critic: round 1 recorded 2026-09-10 in ../audit/CRITIC_LOG.md — 1 fatal fixed. Round 0 (commit 712241c) edits were retained but left no verdict record. -->
