...
**How to read it** — start with the investor rows V03–V10, then the operator rows V06–V08; attack every number against its source artifact and treat `pending-image` as honest absence, not completion.
...
Target: 22 visuals; 8 exact-text HTML infographics, 9 Mermaid/source visuals, 5 image-prompt-only schematic rows. No PNG rows are marked rendered.
...
| ID | Title | Audience | Source artifact | Form | Status |
|---|---|---|---|---|---|
| V06 | CAMIR core loop | Operators | tech/architecture/00_INDEX.md | mermaid | source-mermaid |
| V07 | Ravi's control path from shadow mode to pin | Operators | narrative/pitch_deck.md | html | rendered |
| V08 | Self-hosted cost-quality frontier | Operators | tech/whitepaper.md | mermaid | source-mermaid |
